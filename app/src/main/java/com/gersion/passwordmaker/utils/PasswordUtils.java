...
final public class PasswordUtils {
	...
	private static int getCount(boolean big, boolean small, boolean number,
			boolean chars) {
		int count = 0;
		if(big){
			count+=1;
	...
	public static String getNewPassword(boolean big, boolean small, boolean number,
			boolean chars, int length) {
		setArr(charBigArr);
		setArr(charSmallArr);
		setArr(numArr);
		setArr(charSpecial);
	 ...
	 * @return 密码的长度值
	 */
	public static String getPasswordLevel(boolean isBig, boolean isSmall, boolean isSpecial, boolean isNumber,
									   int length) {
		boolean[] arr = { isBig, isSmall, isSpecial, isNumber };
		int count = 0;
		for (int i = 0; i < arr.length; i++) {
